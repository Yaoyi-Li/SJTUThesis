...
\begin{figure}[t]
	...
	\subfigure[]{
		\centering
		\includegraphics[width=0.45\columnwidth]{chap2/itera_data.pdf}
		\label{fig2:itera}
	}
	\hspace{1em}
	\subfigure[]{
		\centering
		\includegraphics[width=0.45\columnwidth]{chap2/delta_hist.pdf}
		\label{fig2:Hist}
	}
...
由数据矩阵$X$为零均值标准化矩阵，可以得知$\lambda {\bf1}p = {\bf1}XX^Tp = 0$。因而，对于大于$0$的最大特征值，其对应地特征向量$p$总满足${\bf1}p = 0$。将令公式（\ref{eq2:XLX}）中第二部分取得最小值的解$P$写为 $P=(p_1,p_2,...,p_t)$，可以得到
\begin{equation}
	%\begin{split}
