...
\label{eq5:matting}
\end{equation}
其中$ \alpha_i $表示要在像素位置$ i $所估计的alpha遮罩值。如果$ \alpha_i $的取值不是$0$或者$1$，则在像素位置$p$的图像像素值即是由前景和背景混合而成。由于前景颜色$ F_i $、背景颜色$ B_i $和alpha取值$ \alpha_i $都是未知的，所以图像抠图的原始数学定义是一个欠定问题。因此，大多数先前的传统抠图算法都会在抠图问题中引入一个比较强的归纳偏置（inductive bias）。同时，在大多数抠图任务中，都将trimap图像作为粗略标注提供给算法，trimap图像上标注有已知的前景和背景区域以及需要预测的未知区域。
